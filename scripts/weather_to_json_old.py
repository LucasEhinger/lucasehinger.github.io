...
# FXX_LIST = list(range(0, 48 + 1, 1000))  # every 1 hour
# FXX_LIST_GFS = list(range(0, 120 + 1, 2000))  # every 2 hours
# FXX_LIST_NAM = list(range(0, 60 + 1, 1000))  # every 1 hour
FXX_LIST = list(range(0, 1 + 1, 2))  # every 1 hour
FXX_LIST_GFS = list(range(0, 1 + 1, 2))  # every 2 hours
FXX_LIST_NAM = list(range(0, 1 + 1, 2))  # every 1 hour
...
preprocess = joblib.load('/Users/lucasehinger/Library/CloudStorage/OneDrive-Personal/Documents/MIT/lucasehinger.github.io/files/weather/models/preprocessor.pkl')

# Load models
xgb_model = xgb.XGBClassifier()
xgb_model.load_model('/Users/lucasehinger/Library/CloudStorage/OneDrive-Personal/Documents/MIT/lucasehinger.github.io/files/weather/models/xgboost_best_f1.json')
rf_model = joblib.load('/Users/lucasehinger/Library/CloudStorage/OneDrive-Personal/Documents/MIT/lucasehinger.github.io/files/weather/models/random_forest_best_f1.pkl')
gb_model = joblib.load('/Users/lucasehinger/Library/CloudStorage/OneDrive-Personal/Documents/MIT/lucasehinger.github.io/files/weather/models/gradient_boosting_best_f1.pkl')

# Load metadata (thresholds, etc.)
with open('/Users/lucasehinger/Library/CloudStorage/OneDrive-Personal/Documents/MIT/lucasehinger.github.io/files/weather/models/model_metadata.json') as f:
    metadata = json.load(f)

# Convert results to DataFrame
def results_to_dataframe(results, locations):
    """
    Convert the results dictionary to a pandas DataFrame.
    Each row represents one forecast hour for one location.
    """
    rows = []

    # Parse base run date to compute per-forecast month/day
    try:
    ...
    for loc in locations:
        ...
        # Collect all forecast hours present under any variable
        all_fxx = set()
        for label_data in location_results.values():
            if isinstance(label_data, dict):
                all_fxx.update(label_data.keys())

        # Sort forecast hours numerically when possible
        fxx_sorted = sorted(
            all_fxx,
            key=lambda v: int(v) if isinstance(v, (int, str)) and str(v).isdigit() else 0
        )

        for fxx in fxx_sorted:
            # numeric fxx if possible
            try:
            ...
            # Compute month/day based on base_date + fxx hours
    ...
    desired_columns = [
        "fxx", "cloud_top_hrrr", "boundary_layer_cloud_layer", "low_cloud_layer_percent_hrrr",
        "middle_cloud_layer_percent_hrrr", "high_cloud_layer_percent_hrrr", "cloud_ceiling_m_hrrr",
        "cloud_base_m_hrrr", "cloud_top_pres_hrrr", "cloud_base_pres_hrrr", "cloud_top_hgt_hrrr",
        "wind_10m_day_max_hrrr", "tmp_500mb_hrrr", "tmp_700mb_hrrr", "tmp_850mb_hrrr",
        "tmp_925mb_hrrr", "tmp_1000mb_hrrr", "tmp_2m_hrrr", "rh_2m_hrrr", "hpbl_surface_hrrr",
        "hgt_0C_iso_hrrr", "vis_surface_hrrr", "prate_surface_hrrr", "apcp_surface_hrrr",
        "cloud_ceiling_gfs", "low_cloud_layer_percent_gfs", "middle_cloud_layer_percent_gfs",
        "high_cloud_layer_percent_gfs", "boundary_layer_cloud_layer_gfs", "vis_surface_gfs",
        "prate_surface_gfs", "apcp_surface_gfs", "tmp_500mb_gfs", "tmp_700mb_gfs",
        "tmp_850mb_gfs", "tmp_925mb_gfs", "tmp_1000mb_gfs", "tmp_2m_gfs", "rh_2m_gfs",
        "rh_925mb_gfs", "hpbl_surface_gfs", "hgt_0C_iso_gfs", "cloud_ceiling_nam",
        "low_cloud_layer_percent_nam", "middle_cloud_layer_percent_nam", "high_cloud_layer_percent_nam",
        "boundary_layer_cloud_layer_nam", "vis_surface_nam", "tmp_500mb_nam", "tmp_700mb_nam",
        "tmp_850mb_nam", "tmp_925mb_nam", "tmp_1000mb_nam", "tmp_2m_nam", "rh_2m_nam",
        "rh_925mb_nam", "hpbl_surface_nam", "hgt_0C_iso_nam", "prate_surface_nam", "apcp_surface_nam",
...
# Call the function after results are populated
weather_df = results_to_dataframe(results, [LOCATIONS[0]])
