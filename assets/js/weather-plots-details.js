...
const d_modelMarkers = { hrrr: "circle", nam: "square", gfs: "diamond", rap: "triangle-up" };
...
function d_getSelectedModel(){ const selected = Array.from(document.querySelectorAll('input[name="model-toggle"]:checked')).map(el=>el.value); return { hrrr: selected.includes('hrrr'), nam: selected.includes('nam'), gfs: selected.includes('gfs'), rap: selected.includes('rap') }; }
...
function loadDetailsPlotsFromData(data){
  ...
  const showHRRR = !!selectedModel.hrrr; const showNAM = !!selectedModel.nam; const showGFS = !!selectedModel.gfs; const showRAP = !!selectedModel.rap;
  ...
  // Guarded RAP trace builder: returns null when the snapshot predates the RAP
  // fields, so the toggle never breaks a plot for older example days.
  const d_rapTrace = (key, name, color, mapFn, extra) => { const d = data[key]; if(!d||!d.y) return null; return Object.assign({ x:convertedDates, y: mapFn ? d.y.map(mapFn) : d.y, mode:'lines+markers', type:'scatter', name, line:{dash:'dashdot',color}, marker:{symbol:d_modelMarkers.rap} }, extra||{}); };
  ...
  Plotly.newPlot('plot1', traces1, { title:{text:'Cloud Coverage Percentage', font:{color:textColor}}, xaxis:{title:''}, yaxis:{title:'Cloud Coverage (%)', tickfont:{color:textColor}}, legend:{font:{color:textColor}}, showlegend:true });
  ...
  Plotly.newPlot('plot2', traces2, { title:{text:'Cloud Ceiling and Base Height', font:{color:textColor}}, xaxis:{}, yaxis:{title:`Height (${d_heightLabel(selectedUnits)})`}, legend:{font:{color:textColor}}, showlegend:true });
  ...
  Plotly.newPlot('plot3', traces3, { title:{text:'Temperature at Various Isobars', font:{color:textColor}}, xaxis:{}, yaxis:{title:`Temperature (${tempUnitLabel})`}, legend:{font:{color:textColor}}, showlegend:true });
  ...
  Plotly.newPlot('plot5', traces5, { title:{text:'Relative Humidity', font:{color:textColor}}, xaxis:{}, yaxis:{title:'Relative Humidity (%)'}, legend:{font:{color:textColor}}, showlegend:true });
  ...
  Plotly.newPlot('plot6', traces6, { title:{text:'0°C Isotherm Height', font:{color:textColor}}, xaxis:{}, yaxis:{title:`Height (${d_heightLabel(selectedUnits)})`}, legend:{font:{color:textColor}}, showlegend:true });
  ...
  Plotly.newPlot('plot7', traces7, { title:{text:'Surface Visibility', font:{color:textColor}}, xaxis:{}, yaxis:{title:`Visibility (${d_visibilityLabel(selectedUnits)})`}, legend:{font:{color:textColor}}, showlegend:true });
  ...
  Plotly.newPlot('plot10', traces10, { title:{text:'Precipitation', font:{color:textColor}}, xaxis:{}, yaxis:{title:`Accumulated (${d_precipLabel(selectedUnits)})`, rangemode:'tozero'}, yaxis2:{title:`Rate (${d_precipRateLabel(selectedUnits)})`, overlaying:'y', side:'right', rangemode:'tozero'}, legend:{font:{color:textColor}}, showlegend:true });
